...
const TableDemo = () => {
            ...
            <>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-2 p-2">
                    ...
                    <div className="flex flex-col md:flex-row justify-end items-center gap-2 w-full md:w-auto">
                        <Button label="Yazdır" icon="pi pi-print" iconPos="right" className="p-button-outlined w-full md:w-auto" />
                    </div>
                </div>
            </>
        );
    ...
    useEffect(() => {
        ...
        if (selectedDropdownPublisher) {
            ...
            ExamListService.getGeneralExamDetail(selectedDropdownPublisher?.id)
                .then((data) => {
                    console.log('Fetched General Exam Detail:', data);
                    const isGenel = data.details?.type.length > 1 && true;

                    // Handle the fetched exam details
                    const updatedPublishers = [...(isGenel ? [genelOption] : []), ...(data.details?.type || [])]; // Ensure data.details.type is an array
                    setGeneralExams(updatedPublishers); // Update general exams with array

                    setMergedResults(data.details?.mergedResults || []); // Handle mergedResults gracefully
                    ...
                    if (selectedGeneralExam && selectedLesson.length === 0) {
                        ...
                        console.log('lesson not selected datas:', selectedExam);
                    }
                    if (selectedGeneralExam && selectedLesson) {
                        console.log('Selected Exam Data lesson seletectes:', selectedLesson);
                        const selectedExam = data.details?.[selectedGeneralExam.name]; // Access the selected exam array
                        ...
                        if (selectedExam?.length > 0) {
                            ...
                            selectedExam.forEach((examObj: any) => {
                                if (examObj.lessonResults) {
                                    ...
                                    const matchedLesson = examObj.lessonResults.filter((lesson: any) => lesson.lessonId._id === selectedLesson);
                            ...
                            const lessonOptions = selectedExam.flatMap(
                                (examObj: any) =>
                            ...
                            const uniqueLessonOptions = lessonOptions.filter(
                                (lesson: { value: any }, index: any, self: any[]) => index === self.findIndex((l) => l.value === lesson.value) // Ensure the lesson ID is unique
                            );
    ...
    function handleSearch() {
        ...
        if (selectedGeneralExam?.name === 'Genel') {
            ...
            setOriginalTableValues(mergedResults);
        }
        // Second condition: When typeValues is available and selectedLesson is not empty
            ...
            if (selectedLesson.length > 0) {
                ...
                setOriginalTableValues(sortedTypeValues);
                        ...
                        {generalExams.length > 0 && (
                            <>
                                <div className={`col-12 mb-2 lg:col-${selectedGeneralExam?.name === 'Genel' ? '6' : '4'} lg:mb-0`}>
                                    <h5>Deneme Türü Seçiniz *</h5>
                                    <div className="w-full">
                                        <Dropdown value={selectedGeneralExam} onChange={(e) => handleSelectGeneralExam(e.value)} options={generalExams || []} optionLabel="name" placeholder="Deneme Türü Seçiniz" className="w-full" filter />
                                    </div>
                                </div>
                            </>
                        )}
                        {generalExams.length > 0 && selectedGeneralExam?.name == 'Genel' && (
                            <>
                                <div className="col-12 mb-1  lg:col-1 lg:mb-0">
                                    <div className="w-full">
                                        <Button label="Ara" icon="pi pi-search" iconPos="right" className="p-button-outlined w-full" style={{ marginTop: '2.4rem' }} onClick={handleSearch} />
                                    </div>
                                </div>
                            </>
                        )}
                        {lessons.length > 0 && selectedGeneralExam?.name !== 'Genel' && (
                            <>
                                <div className="col-12 mb-2 lg:col-3 lg:mb-0">
                                    <h5>Ders Seçiniz</h5>
                                    <div className="w-full">
                                        <Dropdown value={selectedLesson} onChange={(e) => setSelectedLesson(e.value)} options={lessons || []} optionLabel="label" placeholder="Ders Seçiniz" className="w-full" filter />
                                    </div>
                                </div>
                                <div className="col-12 mb-1  lg:col-1 lg:mb-0">
                                    <div className="w-full">
                        ...
                        {selectedGeneralExam?.name == 'Genel' || selectedLesson.length === 0 ? (
                            ...
                            <>
                                <DataTable
                                    ...
                                    header={header1}
                                >
                                    <Column field="examRank.classRank" body={examRankTemplate} header="Sıra" sortable />
                                    <Column field="name" header="İsim" sortable />
                                    <Column field="schoolNumber" header="Numara" />
                                    <Column
                                        field="class"
